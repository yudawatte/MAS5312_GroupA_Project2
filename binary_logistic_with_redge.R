...
pred_redge <- predict(ridge_model, newx = X_test, type = "response")

# Convert predictions to binary (0/1) keeping threshold to 0.5
pred_cls_redge <- as.numeric(pred_redge > 0.5)

# Calculate accuracy
accuracy_redge <- sum(pred_cls_redge == y_test) / length(y_test)
print(accuracy_redge)

table(pred_cls_redge)
table(y_test)



# Create a confusion matrix
confusion_matrix <- confusionMatrix(data=as.factor(ifelse(pred_redge > 0.5, "1", "0")),
                                    mode = "everything",
                                    positive="1",
...
# Fit the final model using the best lambda
ridge_model_iv <- glmnet(X_iv, y_iv, 
                        family = "binomial", 
                        alpha = 0, 
                        lambda = cv_fit_iv$lambda.min)

print(ridge_model_iv$beta)

# Predict on test data
pred_redge_iv <- predict(ridge_model_iv, newx = X_test, type = "response")

# Convert predictions to binary (0/1) keeping threshold to 0.5
pred_cls_redge_iv <- as.numeric(pred_redge_iv > 0.5)

# Calculate accuracy
accuracy_redge_iv <- sum(pred_cls_redge_iv == y_test) / length(y_test)
print(accuracy_redge_iv)

table(pred_cls_redge_iv)
table(y_test)



# Create a confusion matrix
confusion_matrix_iv <- confusionMatrix(data=as.factor(ifelse(pred_redge_iv > 0.5, "1", "0")), 
                                       mode = "everything",
                                       positive="1",
                                       reference=as.factor(y_test))
